...
module Decidim::Courses
  describe Admin::CreateCourseAdmin do
    subject { described_class.new(form, current_user, my_course) }

    let(:my_course) { create :course }
    let!(:email) { "my_email@example.org" }
    let!(:role) { "admin" }
    let!(:name) { "Weird Guy" }
    let!(:user) { create :user, email: "my_email@example.org", organization: my_course.organization }
    ...
    let(:form) do
      double(
        ...
        role: role,
        name: name,
        current_participatory_space: my_course
      )
    end
    let(:invalid) { false }
    ...
    context "when everything is ok" do
      it "creates the user role" do
        subject.call
        roles = Decidim::CourseUserRole.where(user: user)
      ...
      it "makes the new admin follow the process" do
        ...
        expect(user.follows?(my_course)).to be true
      end

      it "traces the action", versioning: true do
        expect(Decidim.traceability)
          .to receive(:perform_action!)
          .with(:create, Decidim::CourseUserRole, current_user, resource: hash_including(:title))
          .and_call_original
