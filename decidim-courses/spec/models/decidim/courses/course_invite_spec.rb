...
module Decidim
  module Courses
    describe CourseInvite do
      subject { course_invite }

      let(:course_invite) { build_stubbed(:course_invite) }

      it { is_expected.to be_valid }

      it "has an associated course" do
        expect(course_invite.course).to be_a(Decidim::Course)
      end

      it "has an associated user" do
        expect(course_invite.user).to be_a(Decidim::User)
      end

      context "without a course" do
        let(:course_invite) { build :course_invite, course: nil }

        it { is_expected.not_to be_valid }
      end

      context "without an user" do
        let(:course_invite) { build :course_invite, user: nil }

        it { is_expected.not_to be_valid }
      end

      describe "#accept!" do
        let(:course_invite) { create(:course_invite, accepted_at: nil, rejected_at: Time.current) }
      ...
      shared_examples "rejecting and invitation" do |method_name|
        let(:course_invite) { create(:course_invite, accepted_at: Time.current, rejected_at: nil) }
