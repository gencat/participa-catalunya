...
module Decidim::ResourceBanks
  describe Admin::CreateResourceBankAdmin do
    subject { described_class.new(form, current_user, my_resource_bank) }

    let(:my_resource_bank) { create :resource_bank }
    let!(:email) { "my_email@example.org" }
    let!(:role) { "admin" }
    let!(:name) { "Weird Guy" }
    let!(:user) { create :user, email: "my_email@example.org", organization: my_resource_bank.organization }
    let!(:current_user) { create :user, email: "some_email@example.org", organization: my_resource_bank.organization }
    let(:form) do
      double(
        invalid?: invalid,
        email: email,
        role: role,
        name: name,
        current_participatory_space: my_resource_bank
      )
    end
    let(:invalid) { false }
    ...
    context "when everything is ok" do
      it "creates the user role" do
        subject.call
        roles = Decidim::ResourceBankUserRole.where(user: user)
      ...
      it "makes the new admin follow the process" do
        ...
        expect(user.follows?(my_resource_bank)).to be true
      end

      it "traces the action", versioning: true do
        expect(Decidim.traceability)
          .to receive(:perform_action!)
          .with(:create, Decidim::ResourceBankUserRole, current_user, resource: hash_including(:title))
          .and_call_original
